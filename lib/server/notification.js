const sendNotification = (isDebug, fcmConnections, apnsConnections, defaults, userToken, mongoNote) => {
  ...
  if (isDebug) { console.log('Final notification right before shoot out:', JSON.stringify(note, null, 2)) }
  //if (isDebug) { console.log('Final notification right before shoot out:', note) }

  if(userToken.android || userToken.webpush) {
    note.token = (userToken.android || userToken.webpush)
  ...
  if(userToken.apn) {
    ...
    console.log("note.aps",note.apns.payload.aps);
    let notification = new Notification(userToken.apn,{aps: note.apns.payload.aps})
    apnsConnections.send(notification)
      .then(response => {
        console.log("Sucess call to APN");
      })
      .catch(error => {
        console.log("Error from APN:",error);
      })
  }
}
