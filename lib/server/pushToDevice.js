...
Push.Configure = serverConfig => {
  ...
  if(Meteor.isDevelopment) {
    console.log("In development, setting APN accordingly");
    serverConfig.apnConfig.host = 'api.development.push.apple.com';
    // This is a huge fucking hack becuase of fucking apple cunts
    serverConfig.apnConfig.defaultTopic = 'tech.sailstudio.boatmonitor'
  }
  console.log("apnConfig:",serverConfig.apnConfig);
  const apnsConnections = new APNS(serverConfig.apnConfig);

  //console.log("Push: default app:",fcmConnections,apnsConnections);
  if (serverConfig.firebaseAdmin) {
    ...
    self.sendNotification = (userTokens, mongoNote) => sendNotification(isDebug, fcmConnections, apnsConnections, serverConfig.defaults, userTokens, mongoNote)
  }
  ...
  const querySend = (query, mongoNote) => {
    ...
    Push.appCollection.find(query).map(app => {
      ...
      console.log("###",(app.token,mongoNote));
      self.sendNotification(app.token, mongoNote)
    })
